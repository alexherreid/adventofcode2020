...
namespace AdventOfCode2020
{
	public static class Day5
	{
		...
		public static void Day5_Puzzle2()
		{
			Console.Clear();
			Console.WriteLine("Day 5 Challenge 2 Summary: Binary Boarding boarding passes.");
			Console.WriteLine("Given a list of encoded boarding passes, eliminate occupied seats to try to determine which seat is yours.");
			Console.WriteLine("***********************");
			Console.WriteLine("Input(s): List of occupied seats in 10 character chunks.");
			Console.WriteLine("***********************");
			...
			foreach (string bpline in boardingPassesRead)
			{
				...
				//Every seat also has a unique seat ID: multiply the row by 8, then add the column.
				//In this example, the seat has ID 44 * 8 + 5 = 357.
				seatIds.Add((indexRow * 8) + indexSeat);
			}

			//Now we have all the seat IDs in a list. Figure out where we are, it won't be in the list (not occupied by someone else) 
			//but the rules say +1 and -1 from us WILL be in the list.
			int[] seatsSorted = seatIds.OrderBy(x => x).ToArray();
			//The array0 will start at some random number, but after that, any missing number
			//in the sequence would mean it must be your seat!
			int yourSeatId = 0;
			for (int i = 0; i < seatsSorted.Length; i++)
			{
				if (i > 0)//don't process the first item in array
				{
					if (seatsSorted[i] - seatsSorted[i - 1] > 1)
					{
						yourSeatId = seatsSorted[i] - 1;
						break;
					}
				}
			...
			Console.WriteLine();
			Console.WriteLine($"The ID of the missing seat was {yourSeatId}");
